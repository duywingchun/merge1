...
public class PlantGrowth : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            // Chỉ log warning thay vì error, và vô hiệu hóa component để tránh spam log
            Debug.LogWarning($"[PlantGrowth] {gameObject.name}: PlantData is null or invalid! Component disabled.");
            enabled = false;
        }
    }
    ...
    public void StartGrowing()
    {
        ...
        if (plantData == null || !plantData.IsValid())
        {
            Debug.LogWarning($"[PlantGrowth] {gameObject.name}: Cannot start growing - PlantData is null or invalid");
            return;
        }
